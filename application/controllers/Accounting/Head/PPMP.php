...
class PPMP extends MY_Controller{
	...
	public function get_ppmp(){
		$this->load->view($this->layout, ['content' => 'Accounting/content/PPMP', 'js' => 'Accounting/js/PPMP', 'nav' => 'ppmp']);
	}
		...
		$dept_id = $this->session->userdata('department');
	 	...
	 	if($_FILES["file"]["size"] > 0){
		 	$filename=$_FILES["file"]["tmp_name"];
     	 	...
     	 	$ppmp = ['year' => date('Y'), 'quarter' => $quarter, 'dept_id' => $dept_id];
     	 	$items = [];

     	 	while (($emapData = fgetcsv($file, 10000, "\r")) !== FALSE){
	    		foreach ($emapData as $key => $value) {
	    			$item = explode(';', $value);
	    			array_push($items, ['name' => $item[0], 'description' => $item[1], 'qty' => $item[2], 'unit_cost' => $item[3], 'total_cost' => $item[4], 'type' => 'ppmp', 'source' => 'csv', 'item_type' => 'supply']);
	    		}
